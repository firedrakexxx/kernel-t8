...
/* SMDK64XX has a 12MHZ crystal attached to WM8960 */
...
/*	ret = snd_soc_dai_set_pll(codec_dai, WM8960_PLLA, 0,
					SMDK64XX_WM8960_FREQ, pll_out);
	if (ret < 0)
		return ret;

	ret = snd_soc_dai_set_clkdiv(&wm8960_dai[WM8960_DAI_PAIFTX],
					WM8960_BCLKRATIO, bfs);
	if (ret < 0)
		return ret;

	ret = snd_soc_dai_set_clkdiv(&wm8960_dai[WM8960_DAI_PAIFRX],
					WM8960_BCLKRATIO, bfs);
	if (ret < 0)
		return ret;
*/
...
static int smdk64xx_wm8960_init_paiftx(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_dai_set_fmt(&wm8960_dai,//[WM8960_DAI_PAIFTX],
					SND_SOC_DAIFMT_I2S
	...
	/* Set WM8960 to drive MCLK from its PLLA */
/*	ret = snd_soc_dai_set_clkdiv(&wm8960_dai[WM8960_DAI_PAIFTX],
					WM8960_SYSCLK_MCLK, WM8960_CLKSRC_PLLA);
	if (ret < 0)
		return ret;
*/
...
static int smdk64xx_wm8960_init_paifrx(struct snd_soc_codec *codec)
{
	...
	/* Set the Codec DAI configuration */
	ret = snd_soc_dai_set_fmt(&wm8960_dai,//[WM8960_DAI_PAIFRX],
					SND_SOC_DAIFMT_I2S
					 | SND_SOC_DAIFMT_NB_NF
					 | SND_SOC_DAIFMT_CBM_CFM);
	if (ret < 0)
	...
	/* Set WM8960 to drive MCLK from its PLLA */
/*	ret = snd_soc_dai_set_clkdiv(&wm8960_dai[WM8960_DAI_PAIFRX],
					WM8960_MCLK, WM8960_CLKSRC_PLLA);
	if (ret < 0)
		return ret;
*/
...
static struct snd_soc_dai_link smdk64xx_dai[] = {
	{ /* Primary Playback i/f */
		...
		.codec_dai = &wm8960_dai,//[WM8960_DAI_PAIFRX],
	...
	{ /* Primary Capture i/f */
		...
		.stream_name = "Capture",
		.cpu_dai = &s3c64xx_i2s_v4_dai[S3C24XX_DAI_I2S],
		.codec_dai = &wm8960_dai,//[WM8960_DAI_PAIFTX],
