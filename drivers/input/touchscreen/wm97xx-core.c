...
static int abs_x[3] = {100, 3980, 40};
module_param_array(abs_x, int, NULL, 0);
MODULE_PARM_DESC(abs_x, "Touchscreen absolute X min, max, fuzz");

static int abs_y[3] = {200, 3760, 40};
...
/* hcj for dbg */
        ...
        xraw = (data.x & 0xfff);
        yraw = (data.y & 0xfff);
        xtmp = (a*xraw + b*yraw +c)/div;
        ytmp = (d*xraw + e*yraw +f)/div;
...
static int wm97xx_probe(struct device *dev)
{
	...
	*/
	...
	if (ts_proc_entry) {
		printk(KERN_INFO "hey, /proc/driver/micc_ts created!\n");
		ts_proc_entry->write_proc = ts_proc_write;   
	}  

   	input_set_abs_params(wm->input_dev, ABS_X, 0, 800, 0, 0);
	input_set_abs_params(wm->input_dev, ABS_Y, 0, 480, 0, 0);
